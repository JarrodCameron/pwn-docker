...
RUN apt-get install -y apt-utils ascii awscli bat build-essential cargo cmake \
        cscope curl dnsutils entr exuberant-ctags fzf gawk gcc gcc-multilib gdb \
        ...
        libxt-dev locate ltrace make man maven nasm net-tools netcat-openbsd \
        nginx nmap openssh-server pahole patchelf php procps proxychains psmisc \
        python2 python3 python3-dev python3-pip qemu-system ripgrep rubygems \
        strace tmux valgrind vim virtualenvwrapper wget
...
RUN git clone --depth=1 https://github.com/JarrodCameron/.dotfiles \
                /root/.dotfiles \
        && cd /root/.dotfiles \
        && ./restore.sh --force

RUN git clone https://github.com/radare/radare2 /root/tools/radare2 \
        && cd ~/tools/radare2/ \
        && sys/install.sh
